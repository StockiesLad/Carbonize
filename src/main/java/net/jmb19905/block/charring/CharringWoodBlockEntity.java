package net.jmb19905.block.charring;

import net.fabricmc.fabric.api.blockview.v2.RenderDataBlockEntity;
import net.jmb19905.api.FireType;
import net.jmb19905.core.CarbonizeCommon;
import net.jmb19905.core.CharcoalPitInit;
import net.jmb19905.multiblock.CharcoalPitManager;
...
import static net.jmb19905.multiblock.CharcoalPitProviders.createMultiBlock;
...
public class CharringWoodBlockEntity extends BlockEntity implements RenderDataBlockEntity, WrappedQueuer<CharringWoodBlockEntity> {
    ...
    //TODO: make this take in a fireType
    ...
    public void sync(BlockState parent) {
        if (world != null && !world.isClient) {
            if (recipeCache == null)
                recipeCache = world.getRecipeManager().listAllOfType(CarbonizeCommon.BURN_RECIPE_TYPE);
            for (var burnRecipe : recipeCache) for (var fireType : FireType.getAllTypes())
                if (burnRecipe.isInput(parent, fireType) && fireType.isBlockFlammable(parentState)) {
                    this.parentState = parent;
                    this.mediumState = BlockHelper.transferState(burnRecipe.burnBlock().getDefaultState(), parent);
                    this.finalState = BlockHelper.transferState(burnRecipe.successResult().getDefaultState(), parent);
