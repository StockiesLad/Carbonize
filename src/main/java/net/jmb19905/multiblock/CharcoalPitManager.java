...
public class CharcoalPitManager extends PersistentState implements WrappedQueuer<CharcoalPitManager> {
    public static final Identifier CHARCOAL_PIT_ID = new Identifier(CarbonizeConstants.MOD_ID, "charcoal_pit_data");
    ...
    public NbtCompound writeNbt(NbtCompound nbt) {
        ...
        for (int index = 0; index < storage.size(); index++) {
            ...
            if (dataExists) {
                ...
                nbt.putString(withIndex("FireType", index), data.fireType().getSerialId());
    ...
    private static CharcoalPitManager createFromNbt(NbtCompound nbt, ServerWorld world) {
        ...
        for (int index = 0; index < size; index++) {
            if (nbt.getBoolean(withIndex("NotNull", index))) {
                ...
                var fireType = FireType.find(withIndex(nbt.getString("FireType"), index)).orElseThrow();
                ...
                data.storage.add(new CharcoalPitMultiblock(data, blockPositions, fireType, maxBurnTime, burnTime, extinguished));
            }
        }
        return data;
    }
