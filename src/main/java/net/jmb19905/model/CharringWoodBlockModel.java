...
public class CharringWoodBlockModel extends ForwardingBakedModel {
    ...
    //TODO: fix to use other blockstates of ember blocks. And use proper fabric way to retrieve blockstate data.
    @Override
    public void emitBlockQuads(BlockRenderView blockView, BlockState state, BlockPos pos, Supplier<Random> randomSupplier, RenderContext context) {
        var blockEntity = blockView.getBlockEntity(pos, CharcoalPitInit.CHARRING_WOOD_TYPE);
        if (blockEntity.isPresent()) {

            var medium = (BlockState) blockView.getBlockEntityRenderData(pos);
